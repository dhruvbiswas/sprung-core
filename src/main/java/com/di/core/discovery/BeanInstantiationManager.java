...
public class BeanInstantiationManager {
     ...
     */
    public static Object instantiateBean(Class cl, Annotation cl_annotation) throws InstantiationException,
                ...
                for (int j = 0; j < fieldAnnotations.length; j++) {
                    ...
                    if (fieldAnnotations[j].annotationType().getName().
                        ...
                        if (isMandatory) {
                            ...
                            if (DIContainer.DI_CONTAINER.hasBean(
                                    fieldClassName.getName())) {
                                fieldObject = DIContainer.DI_CONTAINER.getBean(
                                        fieldClassName.getName());
                            } else {
                                // the class that this field points to is not in the container
                                fieldObject = instantiateBean(fieldClassName, fieldAnnotations[j]);
                            }
                            f.setAccessible(true);
                            // Set the field 'f' in the object 'o' using dftObject
                            f.set(o, fieldObject);
                            // Lets set the object inside container
                        } else {
